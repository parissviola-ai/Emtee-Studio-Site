...
export default async function LiveSetDevelopmentApplyPage({
  searchParams,
}: {
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden bg-[#0a0709] px-6 py-14 text-white md:py-20">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-24 top-[-90px] h-[340px] w-[340px] rounded-full bg-[radial-gradient(circle,rgba(192,132,252,0.22),transparent_72%)]" />
      </div>
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        <section className="mt-8 rounded-[30px] border border-white/15 bg-black/45 p-8 shadow-[0_24px_72px_rgba(0,0,0,0.5)] backdrop-blur md:p-10">
          ...
          <form className="mt-8 grid grid-cols-1 gap-5" action="/api/ten-ten-entertainment-set-development-apply" method="post">
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">What do you want to improve?</span>
              <textarea name="development_goal" rows={4} required className="mt-2 w-full rounded-xl border border-white/20 bg-black/35 px-4 py-3 text-sm text-white outline-none transition focus:border-white/45 focus:ring-2 focus:ring-white/20" placeholder="Stage presence, transitions, crowd engagement, set flow, etc." />
            </label>

            <div className="mt-1 flex items-center justify-end">
              <button type="submit" className="inline-flex items-center justify-center rounded-full border border-fuchsia-300/40 bg-fuchsia-400/12 px-8 py-3 text-sm font-semibold text-white transition hover:bg-fuchsia-400/20">
                Submit Short Form →
              </button>
            </div>
          </form>
