...
import {
  ROOM_HERO_READY_EVENT,
  ROOM_TRANSITION_HOLD_START_EVENT,
...
export default function RoomsLayout({ children }: { children: ReactNode }) {
  ...
  const [transitionHold, setTransitionHold] = useState<null | {
    fading: boolean;
    objectFit: string;
  ...
  useEffect(() => {
    ...
    const handleLobbyRoomListState = (event: Event) => {
      const open = (event as CustomEvent<{ open?: boolean }>).detail?.open;
      setMobileLobbyShowRoomsOpen(!!open);
    };

    window.addEventListener("emtee:lobby-room-list-state", handleLobbyRoomListState as EventListener);
    return () => {
      window.removeEventListener("emtee:lobby-room-list-state", handleLobbyRoomListState as EventListener);
    };
  }, [isLobby]);
