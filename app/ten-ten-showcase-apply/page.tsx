...
export default async function TenTenShowcaseApplyPage({
  searchParams,
}: {
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden bg-[#08090f] px-6 py-14 text-white md:py-20">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-24 top-[-90px] h-[340px] w-[340px] rounded-full bg-[radial-gradient(circle,rgba(108,92,231,0.22),transparent_72%)]" />
        <div className="absolute right-[-100px] top-[20%] h-[380px] w-[380px] rounded-full bg-[radial-gradient(circle,rgba(56,189,248,0.18),transparent_72%)]" />
      </div>

      <div className="relative mx-auto max-w-4xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link
            href="/tentenentertainment"
        ...
        <section className="mt-8 rounded-[30px] border border-white/15 bg-black/45 p-8 shadow-[0_24px_72px_rgba(0,0,0,0.5)] backdrop-blur md:p-10">
          <p className="text-xs uppercase tracking-[0.22em] text-white/60">Ten Ten Showcase</p>
          <h1 className="mt-3 text-4xl tracking-tight text-white md:text-5xl">Performer Short Form</h1>
          <p className="mt-4 text-white/75">Apply to perform at a Ten Ten Showcase.</p>

          {status === "success" ? (
            <div className="mt-7 rounded-2xl border border-emerald-300/45 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
              Submission received. Ten Ten will follow up.
            </div>
          ) : null}
          {status === "error" ? (
          ...
          <form className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2" action="/api/ten-ten-showcase-apply" method="post">
            ...
            <label className="block md:col-span-2">
              <span className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">Music Link</span>
              <input name="music_link" type="url" required className="mt-2 h-12 w-full rounded-xl border border-white/20 bg-black/35 px-4 text-sm text-white outline-none transition focus:border-white/45 focus:ring-2 focus:ring-white/20" placeholder="Spotify, YouTube, SoundCloud, etc." />
            </label>

            <div className="md:col-span-2 mt-1 flex items-center justify-end">
              <button type="submit" className="inline-flex items-center justify-center rounded-full border border-cyan-300/40 bg-cyan-400/12 px-8 py-3 text-sm font-semibold text-white transition hover:bg-cyan-400/20">
                Submit Application →
              </button>
            </div>
          </form>
